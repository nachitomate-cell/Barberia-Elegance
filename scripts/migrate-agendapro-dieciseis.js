/**
 * migrate-agendapro-dieciseis.js
 *
 * Migra clientes de AgendaPro (Excel .xlsx export, hoja "sheets3") al tenant
 * Studio Dieciséis.
 *
 * ⚠️  El tenantId INTERNO de Studio Dieciséis es 'sionbarberia' (heredado del
 *     rename Sion → Atenas → Dieciséis). El nombre "studiodieciseis" es solo
 *     dominio/marca; en Firestore siempre se usa 'sionbarberia'.
 *
 * Escribe DOS docs por cliente (mismo patrón que migrate-djones-clientes):
 *   tenants/sionbarberia/clientes/{telefonoNormalizado}  — lookup rápido
 *   tenants/sionbarberia/users/{telefonoNormalizado}     — perfil pasivo del Club
 *                                                         (uid = telefono).
 * Como Dieciséis tiene auto-enroll activo, este perfil pasivo es consistente
 * con lo que crearía autoEnrollTenant en la primera cita — la CF hace merge
 * si el cliente vuelve a agendar. Y si el cliente decide registrarse con
 ...
 *
 * Uso:
 *   node scripts/migrate-agendapro-dieciseis.js "ruta/al/archivo.xlsx"
 *   node scripts/migrate-agendapro-dieciseis.js "ruta/al/archivo.xlsx" --commit
 *
 * Si el .xlsx está en la raíz del repo y no se pasa ruta, busca por defecto:
 *   clientes_444596_1782874418 (1).xlsx
 */
...
const DEFAULT_XLSX  = 'clientes_444596_1782874418 (1).xlsx';
...
async function main() {
  ...
  console.log('🎯 Tenant:   ', TENANT_ID, '(Studio Dieciséis)');
  ...
  if (!fs.existsSync(XLSX_PATH)) {
    ...
    console.error('   node scripts/migrate-agendapro-dieciseis.js "ruta/al/archivo.xlsx"');
    process.exit(1);
  }
  ...
  if (!COMMIT) {
    ...
    console.log(`   node scripts/migrate-agendapro-dieciseis.js ${CLI_PATH_ARG ? `"${CLI_PATH_ARG}" ` : ''}--commit\n`);
  }
  process.exit(0);
}
