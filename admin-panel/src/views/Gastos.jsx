      ...
      <div className="relative z-10 w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-primary">Registrar Gasto</h2>
          </div>
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4">
          ...
          <div className="flex items-start justify-between gap-3 bg-slate-800/40 border border-slate-700/60 rounded-lg p-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <p className="text-xs font-semibold text-primary">Repetir cada mes</p>
          ...
          <div className="flex gap-3 justify-end pt-1">
            ...
            <button type="submit" disabled={saving}
              className="flex items-center gap-2 px-5 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-primary text-sm font-semibold rounded-lg transition-all">
...
function KpiCard({ label, value, sub, color = 'red' }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      ...
      <p className={`text-2xl font-bold mt-1 ${color === 'red' ? 'text-red-400' : color === 'amber' ? 'text-amber-400' : 'text-primary'}`}>{value}</p>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {showHelp && (
        <HelpModal title="Ayuda — Gastos" onClose={() => setShowHelp(false)}>
          <p>En <strong className="text-primary">Gastos</strong> registras los costos operativos del local para controlar la rentabilidad.</p>
          <ul className="space-y-1.5 list-disc list-inside text-slate-400">
            <li>Usa <span className="text-primary">Registrar Gasto</span> para agregar un gasto con monto, categoría, descripción y fecha.</li>
            ...
            <li>Compara los gastos con los ingresos de <span className="text-primary">Métricas</span> para calcular tu margen neto.</li>
            <li>Activa <span className="text-primary">Repetir cada mes</span> en gastos fijos (arriendo, luz, internet) y se recrearán solos cada mes. Verás un badge <span className="text-amber-400">Mensual</span> en la plantilla y <span className="text-amber-300">Auto</span> en las copias generadas.</li>
          </ul>
        </HelpModal>
      )}
    </div>
